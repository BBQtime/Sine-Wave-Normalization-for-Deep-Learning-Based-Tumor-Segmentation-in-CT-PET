...
class Autopet_baseline:
    ...
    def load_inputs(self):
        ...
        if ct_mha.endswith('.nii.gz'):
            uuid = os.path.basename(ct_mha)[:-7]  # Remove '.nii.gz'
        else:
            uuid = os.path.splitext(os.path.basename(ct_mha))[0]  # Remove simple extensions like .nii or .mha
    ...
    def predict(self):
        ...
        """
        print("nnUNet segmentation starting!") #--disable_tta
        cproc = subprocess.run(
            f"nnUNetv2_predict -i {self.nii_path} -o {self.result_path} -d 101 -p nnUNetResEncUNetMPlans -tr nnUNetTrainerUmambaSinNorm -c 3d_fullres_bs8 -f all -chk checkpoint_best.pth",
            shell=True,
            check=True,
        )
        print(cproc)
